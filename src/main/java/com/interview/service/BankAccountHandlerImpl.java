...
import com.interview.exceptions.AccountCreationException;
import com.interview.exceptions.DepositException;
import com.interview.exceptions.result.Result;
import com.interview.exceptions.result.Results;
import com.interview.model.bank.Transaction;
import com.interview.model.bank.TransactionType;
import com.interview.model.response.BankResponse;
import com.interview.model.bank.Account;
import com.interview.store.DataStore;
import com.interview.exceptions.BalanceCheckException;
import com.interview.exceptions.WithdrawalException;
...
public class BankAccountHandlerImpl implements IBankAccountHandler {
    ...
    public float getBalance(int bankAccountId) {
        ...
        if (account == null) {
            LOGGER.warn("Rejected: Account " + bankAccountId + " does not exists");
            throw new BalanceCheckException("Rejected: Account " + bankAccountId + " does not exists", Results.results(Result.ACCOUNT_NOT_AVAILABLE));
        }
